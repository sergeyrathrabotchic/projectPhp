...
<?php
    ...
    if(isset($_POST["checkcode"])){
        ...
        $code = htmlspecialchars($_POST["codeProverka"]);
        $vvod_code = (int) $vvod_code;
        $code =  (int) $code;


        if ($vvod_code == $code){
        /*$sql = 'INSERT INTO clients ( name, email) VALUE( "' . $name . '","' . $email . '")';

        $result = mysqli_query($conn, $sql);*/
        $stmt = $pdo->prepare( "INSERT INTO clients ( name, email) VALUE( ?,?)");
  ...
  <?php if ( $registration_email == 1 ) : ?>
        ...
        <br>
        <p>Введите email: <br><input type="email" name="email" style="color: #000" required/>
        </p>
        <br>



            <input type="submit" name="code" value="Отправить" class="Alfabutton" >
...
<?php if ( $registration_code == 1 ) : ?>
        ...
        <input  style="display: none;" type="text" name="codeProverka" value=<?php echo $code; ?> />
        <br>
